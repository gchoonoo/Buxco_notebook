...
format_data = function(data, FUN=log){
  ...
  if(nrow(data) == 0){
    ...
    data_v5$Sex = 'F'
    #capitalize(substring(sapply(strsplit(data_v5$Sample_Name,"_"),"[",2),1,1))
...
mock_mean = function(data){
  ...
  for(i in 1:length(unique(data_v2[,"ID"]))){
    #print(i)
    if(length(data_v2[which(data_v2[,"ID"] == unique(data_v2[,"ID"])[i]),"Days_PI"]) > 1){
  ...
  # get mean AUC for mock
  mock_auc <- ddply(.data=data_v2, .variables=c("Mating","Virus","Date"), .fun=function(x) data.frame(AUC_mock_mean=mean(as.numeric(as.character(x$AUC)), na.rm=T)))
